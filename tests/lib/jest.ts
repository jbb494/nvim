import { object, number, array, string, parse, optional, nullable } from "valibot";
 ...
 */
export async function waitForTestResults(client: NeovimClient, timeoutMs = 10000) {
  ...
  while (Date.now() - startTime < timeoutMs) {
    ...
    // If no tests are running, they've completed
    if (parsed && parsed.running === 0) {
      return;
    }
  ...
  throw new Error(`Tests did not complete within ${timeoutMs}ms`);
}
